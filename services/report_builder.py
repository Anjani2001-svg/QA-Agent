    ...
    for issue in report.get("issues", []):
        lines.append(
            "| {page} | {category} | {severity} | {issue} | {explanation} | {fix} |".format(
                page=issue.get("page_or_section", "").replace("|", "\\|"),
                category=issue.get("category", "").replace("|", "\\|"),
                severity=issue.get("severity", "").replace("|", "\\|"),
                issue=issue.get("issue", "").replace("|", "\\|"),
                explanation=issue.get("explanation", "").replace("|", "\\|"),
                fix=issue.get("recommended_fix", "").replace("|", "\\|")
            )
        )
